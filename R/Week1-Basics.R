...
badcheck<-is.na(x[,1])		##OR
badcheck<-is.na(x$Ozone)
missing<-(x[,1])[badcheck]
length(missing)

oz_new<-(x[,1])[!badcheck] 	##OR
oz_new<-na.omit(x$Ozone)
print(oz_mean<-sum(oz_new)/length(oz_new))

## New way
data_oz <- x[x$Ozone>31,]
data_ozt <- data_oz[data_oz$Temp >90,]
mean(na.omit(data_ozt$Solar.R))

temp_refined<-na.omit(x$Temp[x$Month==6])
print(mean(temp_refined))

oz_refined<-na.omit(x$Ozone[x$Month==5])
print(max(oz_refined))